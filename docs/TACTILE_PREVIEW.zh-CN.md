...
建议把每次触觉测试结果放在 `local_datasets/` 下的独立目录，例如：

```text
local_datasets/tactile_preview/
```
...
```bash
mkdir -p local_datasets/tactile_preview
source scripts/tools/set_replay_env.sh libero /workspace/tabero/local_datasets/tactile_preview
```
...
```bash
find local_datasets/tactile_preview -maxdepth 6 -type f | sort
```

重点看这几个文件：

```text
local_datasets/tactile_preview/replayed_demos/*.hdf5
local_datasets/tactile_preview/video_datasets/*/videos/*.mp4
local_datasets/tactile_preview/video_datasets/*/tactile_outputs/*.mp4
```
...
p = sorted(Path("local_datasets/tactile_preview/replayed_demos").glob("*.hdf5"))[0]
print("file:", p)
...
docker exec tabero-isaaclab bash -lc '
cd /workspace/tabero
mkdir -p local_datasets/tactile_preview/viewer
ffmpeg -y \
  -i local_datasets/tactile_preview/video_datasets/libero_object_task0/videos/demo_0_agentview_rgb.mp4 \
  -i local_datasets/tactile_preview/video_datasets/libero_object_task0/videos/demo_0_eye_in_hand_rgb.mp4 \
  -i local_datasets/tactile_preview/video_datasets/libero_object_task0/tactile_outputs/demo_0_gsmini_left_tactile_rgb.mp4 \
  -i local_datasets/tactile_preview/video_datasets/libero_object_task0/tactile_outputs/demo_0_gsmini_right_tactile_rgb.mp4 \
  ...
  -pix_fmt yuv420p \
  -shortest \
  local_datasets/tactile_preview/viewer/tactile_preview_2x2.mp4
'
```
...
```bash
cat > local_datasets/tactile_preview/viewer/index.html <<'HTML'
<!doctype html>
<html lang="zh-CN">
<head>
  <meta charset="utf-8">
...
<body>
  <main>
    ...
    <p class="meta">
      ...
      数据目录：<code>local_datasets/tactile_preview</code>
    </p>
    <div class="grid">
      <section class="wide">
...
setsid python3 -m http.server 8765 \
  --bind 0.0.0.0 \
  --directory /home/rmc/workspace/Tabero/local_datasets/tactile_preview \
...
```bash
curl -I http://127.0.0.1:8765/viewer/index.html
```
...
```text
local_datasets/tactile_preview/viewer/index.html
local_datasets/tactile_preview/viewer/tactile_preview_2x2.mp4
local_datasets/tactile_preview/replayed_demos/libero_object_task0_pick_up_the_alphabet_soup_and_place_it_in_the_basket_replayed_demo.hdf5
```
